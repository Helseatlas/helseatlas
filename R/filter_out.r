...
filterOut <- function(datasett, filter1=NULL, filter2=NULL, filter3=NULL){
  ...
  if (!is.null(filter1)){
    if ("level1" %in% colnames(datasett)){
      datasett <- try(dplyr::filter(datasett, eval(parse(text ="level1")) == filter1))
    }
  }
  ...
  if (!is.null(filter2)){
    if ("level2" %in% colnames(datasett)){
      datasett <- try(dplyr::filter(datasett, eval(parse(text ="level2")) == filter2))
    }
  }
  ...
  if (!is.null(filter3)){
    ...
    if ("level3" %in% colnames(datasett)){
      datasett <- try(dplyr::filter(datasett, eval(parse(text ="level3")) == filter3))
    }
  }
